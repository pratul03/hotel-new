...
export const paymentSuccessTemplate = (d: {
  guestName: string;
  hotelName: string;
  amount: number;
  bookingId: string;
  ...
  subject: `Payment Receipt — ${formatCurrency(d.amount)} for ${d.hotelName}`,
  html: base(`
    <h2 style="color:#22C55E">✅ Payment Successful!</h2>
    <p>Hi ${d.guestName}, your payment was processed successfully.</p>
    <div class="amount" style="color:#22C55E">${formatCurrency(d.amount)}</div>
...
export const paymentFailedTemplate = (d: {
  guestName: string;
  amount: number;
  bookingId: string;
}) => ({
  ...
  html: base(`
    ...
    <p>Hi ${d.guestName}, your payment of <strong>${formatCurrency(d.amount)}</strong> could not be processed.</p>
    <p>Please retry with a different payment method or contact your bank.</p>
    <a href="${env.APP_URL}/bookings/${d.bookingId}" class="btn">Retry Payment →</a>`),
});
