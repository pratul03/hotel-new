...
export const runSuperhostCalcJob = async (): Promise<void> => {
  const startedAt = new Date();
  const tracker = createJobResult("SuperhostCalc", startedAt);
  let processed = 0;
  const errors: string[] = [];
  ...
  for (const host of hosts) {
    try {
      ...
      const cancelledByHost = await prisma.booking.count({
        where: {
      ...
      const cancellationRate =
        totalBookings > 0 ? (cancelledByHost / totalBookings) * 100 : 0;

      const accountAgeDays =
        (Date.now() - new Date(host.createdAt).getTime()) /
        (1000 * 60 * 60 * 24);
