import { operationsTypeDefs } from "../graphql/schema/typedefs/operations";
...
type GraphQLRequestExample = {
  query: string;
  operationName: string;
  variables?: Record<string, unknown>;
};

type GraphQLResponseExample = {
...
export type GraphQLOperation = {
  ...
  returnType: string;
  requestExample: GraphQLRequestExample;
  responseExample: GraphQLResponseExample;
};
...
const extractTypeBlock = (typeName: "Query" | "Mutation") => {
  const regex = new RegExp(`type\\s+${typeName}\\s*\\{([\\s\\S]*?)\\}`);
  const match = operationsTypeDefs.match(regex);
  return match?.[1] || "";
};
...
const INPUT_EXAMPLES: Record<string, Record<string, unknown>> = {
  ...
  QuickReplyInput: {
    ...
    content:
      "Please carry a valid ID and arrive after 2 PM for smooth check-in.",
    category: "checkin",
  },
...
const RESPONSE_EXAMPLES: Record<string, unknown> = {
  ...
  MfaSetupResult: {
    ...
    otpauthUrl:
      "otpauth://totp/HotelNew:user@example.com?secret=JBSWY3DPEHPK3PXP",
...
const RETURN_SELECTIONS: Record<string, string> = {
  ...
  SearchHotelsResponse:
    "page limit total pages data { id name location instantBooking }",
  Room: "id hotelId roomType maxGuests basePrice amenities",
  Booking:
    "id userId roomId checkIn checkOut guestCount notes amount status createdAt",
  BookingPricePreview:
    "roomId hotelId nights guestCount nightlyBasePrice pricing { subtotal serviceFee tax total }",
  ReservationRisk: "riskScore riskLevel recommendation",
  RebookingOptionsResult:
    "bookingId reason comparableOptions { roomId hotelId basePrice }",
  ...
  PaymentQueueSummary:
    "total pending processing completed failed refunded queued staleProcessing",
  PaymentOrderResult:
    "idempotent order { id amount currency queued } payment { id status amount }",
  ReprocessStalePaymentsResult:
    "dryRun scanned processedCount skippedCount processedPaymentIds skippedPaymentIds",
  ...
  Conversation:
    "userId userName userAvatar lastMessage lastMessageAt unreadCount bookingId",
  ...
  LoyaltySummary:
    "tier rewardPoints totalSpent completedStays referralCode benefits",
  IdentityVerification:
    "userId stage checks { governmentId addressProof selfieMatch } requiredActions",
  ...
  WishlistInviteCreateResult:
    "inviteId shareCode permission invitee { id email name }",
  WishlistAcceptResult: "accepted importedItems listName permission",
  SupportTicket:
    "id userId subject description priority status reply createdAt updatedAt",
  EmergencyTicketResult:
    "escalationStage immediateSteps ticket { id subject priority status }",
  SupportRoutingConsole:
    "generatedAt lookbackDays routingSuggestions { trustAndSafetyPod frontlineSupport }",
  SupportOpsDashboard:
    "generatedAt lookbackDays support { total resolved slaResolutionRate } safety { totalIncidents resolved }",
  IncidentReport:
    "id bookingId reportedByUserId description status resolution resolvedAt createdAt",
  AirCoverBoard:
    "generatedAt incidents { id status } emergencyTickets { id status }",
  OffPlatformFeeCase:
    "id bookingId reporterUserId description status evidenceUrls createdAt",
  HostProfile:
    "id userId companyName website businessType description createdAt updatedAt",
  HostEarningsOverview:
    "totalGross totalServiceFee totalTax totalNet pendingPayoutAmount paidBookingsCount",
  HostTransaction:
    "bookingId createdAt bookingStatus grossAmount paymentStatus serviceFee tax netAmount",
  HostPayoutAccount:
    "id userId accountHolderName bankName accountNumberLast4 ifscCode payoutMethod",
  HostPayoutHistory:
    "availableForPayout payouts { id amount status requestedAt }",
  HostPayout: "id userId amount status notes requestedAt",
  CancellationPolicy:
    "id hotelId policyType freeCancellationHours partialRefundPercent noShowPenaltyPercent",
  QuickReplyTemplate: "id userId title content category createdAt",
  ScheduledMessage:
    "id senderUserId receiverUserId bookingId content sendAt status createdAt",
  HostAnalytics:
    "rangeDays totals { bookings confirmed cancelled revenue avgRating }",
  CoHostAssignment:
    "id hotelId hostUserId cohostUserId permissions revenueSplitPercent createdAt",
  HotelComplianceChecklist:
    "id hotelId jurisdictionCode checklistItems { label completed } status",
  HotelListingToolkit:
    "id hotelId coverImageUrl guidebook houseManual checkInSteps completenessScore",
  HostClaim:
    "id hotelId bookingId hostUserId title description amountClaimed status resolutionNote",
  ComplianceAudit:
    "generatedAt lookbackDays totals { complianceRecords claims } csv",
  Promotion: "code description minSubtotal",
  PromotionValidation: "code description discountAmount subtotal finalSubtotal",
  SearchHistoryItem:
    "id userId queryLocation checkIn checkOut guests createdAt",
  InvoiceDocument:
    "id userId bookingId paymentId type status title documentNumber amount currency fileUrl issuedAt",
  InvoiceAccessUrl: "fileUrl signedUrl expiresInSeconds",
  InvoiceStorageAudit:
    "dryRun repairMissing scanned missingStorageRefs missingObjects repaired failedRepairs",
  Review:
    "id senderId receiverId bookingId hotelId rating comment createdAt updatedAt",
...
const unwrapType = (type: string) => type.replace(/[[\]!\s]/g, "");

// eslint-disable-next-line sonarjs/slow-regex
const isListType = (type: string) => /\[.*\]/.test(type);

const ID_KEYWORD_SAMPLES: Array<[string, string]> = [
  ["booking", "booking_001"],
  ["hotel", "hotel_001"],
  ["room", "room_101"],
  ["payment", "payment_001"],
...
const STRING_KEYWORD_SAMPLES: Array<[string, string]> = [
...
const findKeywordSample = <T>(
  key: string,
  samples: Array<[string, T]>,
): T | undefined => samples.find(([needle]) => key.includes(needle))?.[1];

const sampleScalarValue = (typeName: string, argName?: string): unknown => {
...
const sampleVariableValue = (type: string, argName?: string): unknown => {
...
const sampleResponseValue = (type: string): unknown => {
  const baseType = unwrapType(type);
...
const parseArguments = (argsRaw?: string): GraphQLArgument[] => {
  ...
  return argsRaw
    ...
    .map((chunk) => {
      ...
      return {
        name: match[1],
        type: match[2],
      } satisfies GraphQLArgument;
    })
    .filter((entry): entry is GraphQLArgument => Boolean(entry));
};
...
const buildRequestExample = (
  ...
  return {
    ...
    variables: Object.fromEntries(
      args.map((arg) => [arg.name, sampleVariableValue(arg.type, arg.name)]),
    ),
  };
};

const buildResponseExample = (
  name: string,
  returnType: string,
...
const parseOperations = (kind: OperationKind, block: string) =>
  block
    .split("\n")
    .map((line) => line.trim())
    ...
    .map((signature) => {
      ...
      return {
        ...
        requestExample: buildRequestExample(
          kind,
          parsed.name,
          parsed.args,
          parsed.returnType,
      ...
      } satisfies GraphQLOperation;
    })
    .filter((entry): entry is GraphQLOperation => Boolean(entry));

export const getGraphQLOperationCatalog = (): Catalog => {
...
const toMarkdownRows = (operations: GraphQLOperation[]) =>
  operations
    .map(
      (operation) =>
        `| ${operation.name} | ${escapePipe(operation.signature)} |`,
    )
    .join("\n");

const toExampleSections = (operations: GraphQLOperation[]) =>
  operations
    .map((operation) => {
...
export const getGraphQLOperationMarkdown = () => {
  const { queries, mutations } = getGraphQLOperationCatalog();
