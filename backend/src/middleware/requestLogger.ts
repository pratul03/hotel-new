...
const toSafeLogPayload = (payload: unknown) => {
  try {
...
const parseGraphQLOperation = (body: unknown) => {
...
export const requestLogger = (
  ...
  res.on("finish", () => {
    ...
    console.log(
      `${logLevel} [HTTP] [${id}] ${meta}${gqlMeta} - ${res.statusCode} (${duration}ms)`,
    );

    if (logBodies) {
      console.log(
        `[RES] [${id}] ${meta} body=${toSafeLogPayload(responsePayload)}`,
      );
    }
  });
