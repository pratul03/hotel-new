...
const computeBookingPricing = ({ nightlyBasePrice, nights, serviceFeePercent, taxPercent, }) => {
...
exports.bookingService = {
    ...
    async getBookingPricePreview(payload) {
        if (payload.checkIn >= payload.checkOut) {
            throw new utils_1.AppError("Check-out must be after check-in", 400);
        }
        const room = await database_1.prisma.room.findUnique({
        ...
        const [feeConfig, taxConfig] = await Promise.all([
            database_1.prisma.serviceFeeConfig.findFirst({ orderBy: { createdAt: "desc" } }),
            database_1.prisma.taxConfiguration.findFirst({ where: { region: "default" } }),
        ]);
        const serviceFeePercent = feeConfig?.percentage ?? 13;
        const taxPercent = taxConfig?.taxPercentage ?? 5;
        const pricing = computeBookingPricing({
        ...
        return {
            ...
            cancellationPolicy: room.hotel.cancellationPolicy
                ? {
                    ...
                    freeCancellationHours: room.hotel.cancellationPolicy.freeCancellationHours,
                    partialRefundPercent: room.hotel.cancellationPolicy.partialRefundPercent,
                }
                : null,
        };
    },
    async getReservationRisk(userId, payload) {
        if (payload.checkIn >= payload.checkOut) {
            throw new utils_1.AppError("Check-out must be after check-in", 400);
        }
        const [user, room, cancelledBookings] = await Promise.all([
            database_1.prisma.user.findUnique({ where: { id: userId } }),
            database_1.prisma.room.findUnique({ where: { id: payload.roomId } }),
        ...
        const now = new Date();
        const accountAgeDays = Math.max(0, Math.floor((now.getTime() - user.createdAt.getTime()) / (24 * 3600000)));
        const leadTimeDays = Math.max(0, Math.floor((payload.checkIn.getTime() - now.getTime()) / (24 * 3600000)));
        const [feeConfig, taxConfig] = await Promise.all([
            database_1.prisma.serviceFeeConfig.findFirst({ orderBy: { createdAt: "desc" } }),
            database_1.prisma.taxConfiguration.findFirst({ where: { region: "default" } }),
        ]);
        ...
        const factors = {
            ...
            highCancellationHistory: cancelledBookings >= 3 ? 25 : cancelledBookings * 7,
            veryShortLeadTime: leadTimeDays < 2 ? 20 : leadTimeDays < 5 ? 10 : 0,
            highGuestToCapacityRatio: payload.guestCount / Math.max(1, room.maxGuests) > 0.9 ? 10 : 0,
            highOrderValue: pricing.total > 50000 ? 15 : pricing.total > 25000 ? 8 : 0,
            trustedAccountAge: accountAgeDays >= 365 ? -15 : accountAgeDays >= 90 ? -7 : 0,
        };
    ...
    async getRebookingOptions(userId, bookingId, reason) {
        ...
        const candidates = await database_1.prisma.room.findMany({
            where: {
        ...
        const options = candidates.map((room) => ({
            ...
            estimatedPriceDifference: Number((room.basePrice - booking.room.basePrice).toFixed(2)),
        }));
        return {
            bookingId,
            reason,
    ...
    async createTravelDisruptionCase(userId, bookingId, payload) {
        const booking = await database_1.prisma.booking.findUnique({
            where: { id: bookingId },
        ...
        const rawPercent = baseRefundBySeverity[payload.severity] + eventBoost[payload.eventType];
        const refundPercent = Math.min(100, Math.max(0, rawPercent));
        const estimatedRefundAmount = Number(((booking.amount * refundPercent) / 100).toFixed(2));
        const recommendation = payload.severity === "critical" || payload.eventType === "medical"
            ? "priority_manual_review"
            : "auto_apply_policy";
        const record = await database_1.prisma.travelDisruptionCase.create({
            data: {
                bookingId,
                userId,
    ...
    async simulateTravelDisruptionPolicyMock(userId, bookingId, payload) {
        return this.createTravelDisruptionCase(userId, bookingId, payload);
    },
    async getReservationRiskMock(userId, payload) {
        return this.getReservationRisk(userId, payload);
    },
    async getRebookingOptionsMock(userId, bookingId, reason) {
        return this.getRebookingOptions(userId, bookingId, reason);
    },
    async assessTravelDisruption(userId, bookingId, payload) {
        return this.createTravelDisruptionCase(userId, bookingId, payload);
    },
